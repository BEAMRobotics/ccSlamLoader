...
bool data_model::saveFile(std::string fn) {
  // boost::property_tree::write_xml(fn, pt_, std::locale(),
  // boost::property_tree::xml_writer_make_settings<std::string>(' ', 1u));
  boost::property_tree::write_xml(
      fn, pt_, std::locale(),
      boost::property_tree::xml_writer_make_settings<std::string>('\t', 1));

  //        boost::property_tree::write_xml(fn, pt_, std::locale(),
  //        boost::property_tree::xml_writer_make_settings<char>(' ', 1u));
  xmlPath = boost::filesystem::path(fn);
  return true;
}

bool data_model::getAffine(std::string scanId, Eigen::Matrix4f& matrix) {
  if (!checkIfExists("Model.Transformations." + scanId + ".Affine.Type"))
    return false;
  if (!checkIfExists("Model.Transformations." + scanId + ".Affine.Data"))
    return false;

  std::string type =
      pt_.get<std::string>("Model.Transformations." + scanId + ".Affine.Type");
  std::string data =
      pt_.get<std::string>("Model.Transformations." + scanId + ".Affine.Data");
...
bool data_model::getPointcloudName(std::string scanId, std::string& fn) {
  if (!checkIfExists("Model.Transformations." + scanId + ".cloudname"))
    return false;
...
bool data_model::getAffine(std::string scanId, Eigen::Vector3f& origin,
...
// bool data_model::getAffine (std::string scanId, Eigen::Matrix4f &matrix);
///// loads affine transform with given scan ID
// bool data_model::getAffine (std::string scanId, Eigen::Vec3f &origin,
// Eigen::Vec3f &quat);
...
void data_model::setAffine(std::string scanId, Eigen::Matrix4f& matrix) {
  ...
  std::stringstream ss;
  ss << matrix(0, 0) << " " << matrix(1, 0) << " " << matrix(2, 0) << " "
     << matrix(3, 0) << " ";
  ss << matrix(0, 1) << " " << matrix(1, 1) << " " << matrix(2, 1) << " "
     << matrix(3, 1) << " ";
  ss << matrix(0, 2) << " " << matrix(1, 2) << " " << matrix(2, 2) << " "
     << matrix(3, 2) << " ";
  ss << matrix(0, 3) << " " << matrix(1, 3) << " " << matrix(2, 3) << " "
     << matrix(3, 3) << " ";
  // ss << matrix(0,0) << " "<< matrix(1,0) << " "<< matrix(2,0) << " "<<
  // matrix(3,0) << " "; ss << matrix(0,1) << " "<< matrix(1,1) << " "<<
  // matrix(2,1) << " "<< matrix(3,1) << " "; ss << matrix(0,2) << " "<<
  // matrix(1,2) << " "<< matrix(2,2) << " "<< matrix(3,2) << " "; ss <<
  // matrix(0,3) << " "<< matrix(1,3) << " "<< matrix(2,3) << " "<< matrix(3,3)
  // << " ";
  pt_.put("Model.Transformations." + scanId + ".Affine.Data", ss.str());
}

void data_model::setAffine(std::string scanId, Eigen::Vector3f origin,
                           Eigen::Quaternionf quat) {
  // check if node exists
  addIfNotExists("Model.Transformations." + scanId);

  pt_.put("Model.Transformations." + scanId + ".Affine.Type",
          "Vector3f_Quaternionf");
  std::stringstream ss;
  ss << origin[0] << " " << origin.x() << " " << origin.y() << " " << quat.z()
...
void data_model::addIfNotExists(std::string path) {
  boost::optional<boost::property_tree::ptree&> opt_loc_pt_ =
      pt_.get_child_optional(path);

  if (!opt_loc_pt_) { pt_.add_child(path, boost::property_tree::ptree()); }
}
...
void data_model::getAllScansId(std::vector<std::string>& ids) {
  ...
  BOOST_FOREACH (boost::property_tree::ptree::value_type& v,
                 pt_.get_child("Model.Transformations")) {
...
void data_model::addAlgorithmParam(std::string paramName,
...
void data_model::setResult(std::string scanId, std::string resultName,
                           float result) {
  pt_.put<float>("Model.Algorithms.results." + scanId + "." + resultName,
                 result);
}
...
std::string data_model::getFullPathOfPointcloud(std::string id) {
  ...
  boost::filesystem::path t =
      boost::filesystem::complete(xmlPath).parent_path();
  t /= boost::filesystem::path(dataPath);
  t /= fileName;
  return t.string();
}
// void data_model::setAffine (std::string scanId, Eigen::Vec3f &origin,
// Eigen::Vec3f &quat); void data_model::setPointcloudName (std::string scanId,
// std::string &fn);

void data_model::getResult(std::string scanId, std::string resultName,
                           float& result) {
  result =
      pt_.get<float>("Model.Algorithms.results." + scanId + "." + resultName);
}
void data_model::getGlobalModelMatrix(Eigen::Matrix4f& matrix) {
  ...
  std::string type =
      pt_.get<std::string>("Model.GlobalTransformation.Affine.Type");
  std::string data =
      pt_.get<std::string>("Model.GlobalTransformation.Affine.Data");
...
void data_model::setTimestamp(std::string scanId, boost::posix_time::ptime ts) {
  pt_.put("Model.Transformations." + scanId + ".timestamp",
          boost::posix_time::to_iso_string(ts));
}

bool data_model::getTimestamp(std::string scanId,
                              boost::posix_time::ptime& ts) {
  if (!checkIfExists("Model.Transformations." + scanId + ".timestamp"))
    return false;
  std::string isoTime;
