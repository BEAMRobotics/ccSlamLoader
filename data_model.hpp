...
public:
  ...
  /// loads affine transform with given scan ID
  bool getAffine(std::string scanId, Eigen::Vector3f& origin,
  ...
  /// sets affine to pointcloud wh given id
  void setAffine(std::string scanId, Eigen::Vector3f origin,
                 Eigen::Quaternionf quat);
  ...
  void setResult(std::string scanId, std::string resultName,
                 std::string result);
